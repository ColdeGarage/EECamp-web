...
<template>
  <v-card class="pa-10 ma-0" elevation="7" max-width="1400" width="100%">
    <v-sheet width="100%" class="d-flex justify-end">
      <v-btn v-if="editable" @click="edit = !edit">
    ...
    <div class="d-flex justify-center" v-if="edit">
      <v-btn @click="handleSubmit" color="primary" large>更新報名資料</v-btn>
    </div>
  </v-card>
</template>
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      homeNumber: [
        (v) => !!v || '請輸入住家電話',
        (v) => /^\d{2,4}-\d{7,8}$/.test(v) || '請輸入正確住家電話'
      ],
      phoneNumber: [
        (v) => !!v || '請輸入手機號碼',
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (validateResult) {
        ...
        try {
          delete this.form.alternateNum;
          delete this.form.isAdmin;
          delete this.form.status;
          await this.$api.user.modifyCurrentUser(this.form);
        } catch (error) {
          console.error(error);
          this.alert = true;
          if (
            ...
            this.alertMessage = '資料重複，請再次確認。';
          else
            this.alertMessage =
              '更新失敗，請再次確認報名資料。如你認為結果有誤，請洽主辦單位。';
          return;
        }
        this.edit = false;
        this.success = true;
